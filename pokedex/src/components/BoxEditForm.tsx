...
import '../App.css';
import type { BoxCardProps, InsertBoxEntry } from '../types/types';
import Modal from './Modal';

export function BoxEditForm(props: BoxCardProps) {
    const [location, setLocation] = useState(props.entry.location);
    const [level, setLevel] = useState<number | ''>(props.entry.level);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const submitForm = async () => {
            try {
                ...
                const response = await PokemonAPI.updateBoxEntry(String(props.entry.id), formData);
                console.log('Box entry updated:', response);
            } catch (err) {
